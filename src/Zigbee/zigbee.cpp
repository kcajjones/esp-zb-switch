...
static void taskZigbeeMain(void *pvParameters) {
    ...
    // Initialize Zigbee configuration for a router
    esp_zb_cfg_t zb_nwk_cfg = ESP_ZB_ZCZR_CONFIG();  // Changed from ESP_ZB_ZED_CONFIG to ESP_ZB_ZCZR_CONFIG
    ...
    switch (sig_type) {
        ...
        case ESP_ZB_BDB_SIGNAL_STEERING:
            if (err_status == ESP_OK) {
                ...
                log_w("Network steering was not successful (status: %s)", esp_err_to_name(err_status));
                log_i("Retrying network steering...");
                esp_zb_scheduler_alarm((esp_zb_callback_t)bdb_start_top_level_commissioning_cb, ESP_ZB_BDB_MODE_NETWORK_STEERING, 1000);
            }
            break;
